...
##' @param P a preconditioner (object of class "amgcl")
...
##' @param max_level maximum number of levels in the multiscale hierarchy (default 4)
...
precond <- function(A,tol=1e-5,maxiter=100,coarsen="smoothed_aggregation",relax="spai0",max_level=4,solver="gmres")
{
    ...
    #R gives us CSC format, amgcl wants CSR
    At <- Matrix::t(A)
    xptr <- amgsolver(nrow(A),At@p,At@i,At@x,tol=tol,maxiter=maxiter,coarsening=coarsen,relax=relax,solver=solver)
